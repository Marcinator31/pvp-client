...
public class PvpClientClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // Accounts-Button im Hauptmenue (TitleScreen). Der Keybind tickt im
        // Menue nicht, deshalb hier ein echter Button -- so ist der Account-
        // Switcher auch vor dem Beitreten einer Welt erreichbar.
        net.fabricmc.fabric.api.client.screen.v1.ScreenEvents.AFTER_INIT.register(
            (client, screen, scaledWidth, scaledHeight) -> {
                if (screen instanceof net.minecraft.client.gui.screen.TitleScreen) {
                    net.minecraft.client.gui.widget.ButtonWidget btn =
                        net.minecraft.client.gui.widget.ButtonWidget.builder(
                            net.minecraft.text.Text.literal("Accounts"),
                            b -> client.setScreen(new AccountScreen())
                        ).dimensions(scaledWidth / 2 - 100, scaledHeight / 4 + 96,
                                     200, 20).build();
                    net.fabricmc.fabric.api.client.screen.v1.Screens
                        .getButtons(screen).add(btn);
                }
            });
