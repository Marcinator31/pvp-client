...
public class EspScreen extends Screen {
    ...
    // Ein Eintrag: Entity-Typ + sein Spawn-Egg-Item + die ID als String.
    private static final class MobEntry {
        final SpawnEggItem egg;
        final String id;
        final String name;
        MobEntry(SpawnEggItem egg, String id, String name) {
            this.egg = egg; this.id = id; this.name = name;
        }
    }
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        for (MobEntry mob : mobs) {
            ...
            int iy = y + (CELL - 16) / 2;
            try {
                ctx.drawItem(new ItemStack(mob.egg), ix, iy);
