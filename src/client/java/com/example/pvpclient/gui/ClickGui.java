        ...
        for (Module.Category cat : Module.Category.values()) {
            ...
            for (Module m : ModuleManager.INSTANCE.getByCategory(cat)) {
                ...
                if (expanded.contains(m)) {
                    ...
                    // Block-ESP: Eintrag, der das Block-Auswahl-Menue oeffnet.
                    if (m instanceof com.example.pvpclient.module.modules.BlockEspModule) {
                        ctx.fill(x, y, x + PANEL_W, y + ROW_H, t.accent.get());
                        ctx.drawTextWithShadow(this.textRenderer,
                                Text.literal("> Bloecke auswaehlen"),
                                x + 8, y + 3, t.text.get());
                        y += ROW_H;
                    }
                    if (m instanceof com.example.pvpclient.module.modules.BlockEspModule) {
        ...
        for (Module.Category cat : Module.Category.values()) {
            ...
            for (Module m : ModuleManager.INSTANCE.getByCategory(cat)) {
                ...
                if (expanded.contains(m)) {
                    ...
                    // Block-ESP: Eintrag oeffnet das Block-Auswahl-Menue.
                    if (m instanceof com.example.pvpclient.module.modules.BlockEspModule) {
                        if (inside(mouseX, mouseY, x, y, PANEL_W, ROW_H)) {
                            net.minecraft.client.MinecraftClient.getInstance()
                                .setScreen(new BlockEspScreen(this));
                            return true;
                        }
                        y += ROW_H;
                    }
