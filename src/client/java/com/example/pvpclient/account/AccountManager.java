...
public final class AccountManager {
    ...
    public void switchTo(Account account) {
        ...
        try {
            // Offline-Session: UUID wird deterministisch aus dem Namen abgeleitet
            // (genau wie Vanilla das fuer Offline-Spieler macht). Als accessToken
            // nutzen wir einen Platzhalter -- fuer Offline-/Cracked-Server reicht
            // das, echte Online-Server lehnen die Session ohnehin ab.
            java.util.UUID uuid = net.minecraft.util.Uuids.getOfflinePlayerUuid(account.username);

            net.minecraft.client.session.Session session =
                new net.minecraft.client.session.Session(
                    account.username,
                    uuid,
                    "0",                       // accessToken (Platzhalter)
            ...
            // uuid im Account fuer die Anzeige nachziehen.
